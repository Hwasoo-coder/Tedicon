...
function onPlayerEnter(userID)
    ...
    Debug.Log(" 로컬 플레이어 Stage 2 아이템 획득!")
    -- 2. 데이터 업데이트 (Global 테이블을 통해 GameState 접근)
    if Global.GameState ~= nil then
        Global.GameState.AddItem(2)
        -- 현재 아이템 개수를 가져와서 메시지에 포함
        local currentCount = Global.GameState.GetItem(2)
        UI.ToastMessage("사과 아이템을 획득했습니다! 현재 갯수: " .. tostring(currentCount), 5.0)
        Debug.Log("데이터 업데이트 완료 (Stage 2)")
    else
    ...
    if Global.CheckStage2Clear ~= nil then
        Global.CheckStage2Clear()
        Debug.Log("[Stage2Item] 스테이지 클리어 함수 실행 완료")
    end
    ...
    if vObject ~= nil then
        ...
        Debug.Log("[Stage2Item] VObject 이동 완료")
    else
        -- VObject가 아닌 일반 오브젝트인 경우 로컬 비활성화
        self.gameObject:SetActive(false)
        Debug.Log("[Stage2Item] 일반 오브젝트 비활성화 완료")
    end
end